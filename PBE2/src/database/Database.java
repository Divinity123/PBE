...
public class Database implements DB_operations {
	...
	/* class constructor */
	public Database(Permutation sigma)
	{
		this.sigma = sigma;
	}
	...
	public int[] getMergeSize(int[] minSizes, int[] maxSizes, Integer base) {
		int[] solution = new int[minSizes.length];
		
		for (int ii = 0; ii < solution.length; ii++)
		{
			solution[ii] = (int) Math.pow(base, Math.ceil(Math.log(minSizes[ii]) / Math.log(base)));
	...
	public void getEncodings() {
		...
		mergeSizes = getMergeSize(minSizes, maxSizes, base); // does not deal with empty encodings just yet
	...
	/* method to suppress alphabet size in a column to desired number */
	public void suppress(int col, int size)
	{
		/* change merge sizes and merge shifts */
		mergeSizes[col] = size;
		mergeShifts[mergeShifts.length - 1] = 1;
		for (int ii = mergeShifts.length - 2; ii > - 1; ii--)
			mergeShifts[ii] = mergeShifts[ii+1] * mergeSizes[ii+1];
		...
		/* initialise objects */
		Integer[] binCaps = new Integer[mergeSizes[col]];
		HashMap<String, Integer> encoding = new HashMap<String, Integer>(); 
		
		/* fill all bins once before running the algorithm */
		for (int fill = 0; fill < mergeSizes[col]; fill++)
		{
			binCaps[fill] = attCountsCols.get(col)[fill].getCount();
			encoding.put(attCountsCols.get(col)[fill].getString(), fill);
		}
		
		/* fit into the next bin that can contain the item without increasing the cap */
		for (int idx = mergeSizes[col]; idx < attCountsCols.get(col).length; idx++)
		{
			int currentBin = 1;
			while (binCaps[currentBin] + attCountsCols.get(col)[idx].getCount() > binCaps[0])
			{
				currentBin++;
				if (currentBin >= mergeSizes[col])
					break;
			}
			
			/* if there is a bin to fit, fit it */
			if (currentBin < mergeSizes[col])
			{
				binCaps[currentBin] += attCountsCols.get(col)[idx].getCount();
				encoding.put(attCountsCols.get(col)[idx].getString(), currentBin);
			}
			
			/* otherwise find the bin that has least expansion */
			else
			{
				Integer minBin = 0;
				Integer minCap = Integer.MAX_VALUE;
				
				for (currentBin = 0; currentBin < mergeSizes[col]; currentBin++)
				{
					if (binCaps[currentBin] + attCountsCols.get(col)[idx].getCount() < minCap)
					{
						minBin = currentBin;
						minCap = binCaps[currentBin] + attCountsCols.get(col)[idx].getCount();
					}
				}
				
				binCaps[minBin] += attCountsCols.get(col)[idx].getCount();
				encoding.put(attCountsCols.get(col)[idx].getString(), minBin);
			}
		}
		
		
		encodings.remove(col);
		encodings.add(col, encoding);
	}
	...
	public void padding()
	{
		...
		for (int col = 0; col < mergeSizes.length; col++)
		{
			...
			else
			{
				...
				Solver solver = new Solver(colHist, new Integer[]{mergeSizes[mergeSizesMaxIdx], mergeSizes[col]}, sigma);
				solver.solve();
